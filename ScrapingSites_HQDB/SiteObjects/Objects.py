...
class Venue(object):
    ...
    def __reValidInfo(self,keepPriceList=False):        
        ...
        if keepPriceList != 'keepformatAddress':
            self.getFullAddress()
        ...
        self.pricelist_link = Validator.RevalidURL(self.pricelist_link)                  
    ...
    def toJSON(self,keepPriceList=False):            
        ...
        order = OrderedDict([('name', self.name), ('scrape_page', self.scrape_page), ('street', self.street), ('city', self.city), 
                             ...
                             ('description', self.description), ('services', services_tmp)])
    ...
    def writeToFile(self, folder, index, spec=False,keepPriceList=False):
        try:
            index = '%0*d' % (6, index) 
            outputFile = folder + "/" + index + "_" + Validator.RevalidName(self.name) + ".json"
            if spec == True:
    ...
    def getFullAddress(self):            
        ...
        if self.city != None and self.city.strip() != "":
            if self.formatted_address == None:
                ...
                self.formatted_address = self.formatted_address.strip().rstrip(self.city).strip()
                self.formatted_address += ", " + self.city
        if self.zipcode != None and self.zipcode.strip() != "":
            if self.formatted_address == None:
                self.formatted_address = self.zipcode
            else: 
                self.formatted_address = self.formatted_address.strip().rstrip(self.zipcode).strip()
                self.formatted_address += ", " + self.zipcode
